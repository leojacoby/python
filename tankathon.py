...
for i in range(100001):
    ...
    #recieved_picks is a dictionary with key = seed, value = pick
    recieved_picks = {}
    ...
    for pick, num in zip(draft, range(1, 15)):
        if pick[-1] == '*':
            current_sim.append(num)
    all_sims.append(tuple(current_sim))

outcomes = Counter(all_sims)

for outcome in outcomes.most_common(1000):
    if len(outcome[0]) == 1:
        print('The 76ers got just the {} pick {}% of the time.'.format(outcome[0][0], round(outcome[1]/1000, 1)), end='\n\n')
    else:
        print('The 76ers got the {} and {} picks {}% of the time.'.format(outcome[0][0], outcome[0][1], round(outcome[1]/1000, 1)), end='\n\n')
